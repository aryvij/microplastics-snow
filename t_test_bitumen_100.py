...
file_name = 'output_bitumen_100.txt'
with open(file_name,'w') as f:
    ...
    f.write ("Comparison Umeå-Lueå-100 bitumen")
    f.write (res.summary)
    ...
    data_1 = data_High_traffic_intensity_bitumen_100.values.tolist()
    ...
    data_2 = data_Low_traffic_intensity_bitumen_100.values.tolist()

    new_data = DataFrame({'High':data_1,'Low':data_2})
    res= stat()
    res.ttest(df=new_data, res=['High', 'Low'], test_type=3)

    f.write ("Comparison HT-LT-100 bitumen")
    f.write (res.summary)
    ...
    data_1 = data_Distance_from_intersection_D1_bitumen_100.values.tolist()
    ...
    data_2= data_Distance_from_intersection_D2_bitumen_100.values.tolist()

    new_data = DataFrame({'D1':data_1,'D2':data_2})
    res= stat()
    res.ttest(df=new_data, res=['D1', 'D2'], test_type=3)

    f.write ("Comparison D1-D2-100 bitumen")
    f.write (res.summary)

    #4.DISTANCE_FROM_EDGE_OF_ROAD

    #Distance_from_edge_of_road_BITUMEN_100
    df = pd.read_csv('MP_Data.csv')
    data_Distance_from_edge_point_five_bitumen_100 =  df [df.Distance_from_road == 0.5]  [df.Mesh_size == 100] [df.MP_category == 'Bitumen']['Concentration']
    data_1 = data_Distance_from_edge_point_five_bitumen_100.values.tolist()

    #Distance_from_edge_2.5_BITUMEN_100

    df = pd.read_csv('MP_Data.csv')
    data_Distance_from_edge_two_point_five_bitumen_100 =  df [df.Distance_from_road == 2.5]  [df.Mesh_size == 100] [df.MP_category == 'Bitumen']['Concentration']
    data_2= data_Distance_from_edge_two_point_five_bitumen_100.values.tolist()

    new_data = DataFrame({'0.5':data_1,'2.5':data_2})
    res= stat()
    res.ttest(df=new_data, res=['0.5', '2.5'], test_type=3)

    f.write ("Comparison 0.5-2.5-100 bitumen")
    f.write (res.summary)

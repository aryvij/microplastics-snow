...
def plot_histogram(fig, ax,data,MP_category,mesh_size):
    ...
    data =  data [data.City == 'Umea']['Concentration']
    #print (data)
...
def plot_all_graphs(data):
    ...
    #fig.text(0.5, 0.01, 'count' , va='center', rotation='horizontal', size = 14)
    fig.text(0.25, 0.95, 'Histogram_TRWPs_Umeå' , va='center', rotation='horizontal', size = 16, weight="bold")
    #handles, labels = ax.get_legend_handles_labels()
    #fig.legend(handles, labels, loc='upper right')
    ...
    plt.savefig('Histogram_Umeå.png')
